/* React */
import React from 'react';

/* React-Redux */
import {
    useDispatch
} from 'react-redux';

import {
    setImage
} from 'src/redux/features/imageSlice';

/* Ionic */
import {
    IonFabButton,
    IonIcon
} from '@ionic/react';

import {
    cameraOutline
} from 'ionicons/icons';

/* Capacitor */
import {
    Camera,
    CameraResultType,
    CameraSource
} from '@capacitor/camera';

/* Stylesheet */
import styles from './CapacitorCamera.module.scss';

/* Interface(s) */
interface CapacitorCameraProps {
    handleShowFabList: Function;
}

const CapacitorCamera: React.FC<CapacitorCameraProps> = ({ handleShowFabList }) => {
    const dispatch = useDispatch();

    const captureImage = async () => {
        try {
            const image = await Camera.getPhoto({
                resultType: CameraResultType.DataUrl,
                source: CameraSource.Camera,
                quality: 100
            });

            if (image.dataUrl) {
                dispatch(setImage(image.dataUrl));
            }
        } catch (error: any) {
            console.log(error.message);
        }
    };

    return (
        <IonFabButton
            className={styles.capacitor_camera_fab}
            onClick={
                () => {
                    captureImage();
                    handleShowFabList(false);
                }
            }>
            <IonIcon className={styles.icon} icon={cameraOutline} />
        </IonFabButton>
    );
};

export default CapacitorCamera;